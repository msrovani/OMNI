export class StochasticOptimizer {
    ...
    optimize(assets, priceForecast, currentSoc, objective = "balanced", submercado) {
        const commands = [];
        let totalProfit = 0;
        let totalDegradation = 0;
        for (const asset of assets) {
            const soc = currentSoc[asset.id] ?? 50;
            if (soc < asset.minSocPercent + 5)
                continue;
            const bestTrade = this.findBestTrade(asset, priceForecast, soc, objective);
            if (bestTrade) {
    ...
    findBestTrade(asset, prices, currentSoc, objective) {
    ...
    score(objective, profit, degradation) {
        switch (objective) {
            case "profit":
                return profit;
            case "battery_life":
    ...
    estimateDegradation(asset, dodPercent) {
